...
### entropy(list)
The entropy function takes a list of probabilities and uses it to calculate entropy for a descriptive feature or overall entropy for a target. A list `x` would be used as follows: ![equation1](https://latex.codecogs.com/svg.latex?\dpi{200}H(Feature,\mathcal{D})=-\sum_{i}^{n}x_i*log(x_i))
